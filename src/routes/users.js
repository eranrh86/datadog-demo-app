...
router.get('/', asyncHandler(async (req, res) => {
  // TEST ERROR 7: Comparing with null (fixed for lint)
  if (req.query.filter === null) {
    // Do nothing
  }

  // TEST ERROR 8: Using var instead of const/let (demo)
  const _tempData = 'old style variable';
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  // TEST ERROR 9: No input sanitization (demo - intentional)
  const _unsafeInput = req.body.description;

  // TEST ERROR 10: Weak password check
  if (req.body.password && req.body.password.length < 3) { // Too weak, should be 8+
